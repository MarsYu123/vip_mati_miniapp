// pages/long_vip_pay/long_vip_pay.js
var app = getApp()
...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      new_money: num_transition.num_slice(app.advert.vip_price)
    })
  ...
  go_pay: function () {
    ...
    if (app.open_user.xcx_openid) {
      if(app.open_user.has_team || app.open_user.isFinalVIp){
        var tips = ''
        if(app.open_user.isFinalVIp){
            tips = '您已是永久VIP，请勿重复购买'
        }else if (app.open_user.has_team){
            tips = '您已参与永久VIP拼团活动，请勿重复购买'
        }
          wx.showToast({
            title: tips,
            icon: 'none',
